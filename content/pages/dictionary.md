---
title: "📖 Jargon Dictionary"
date: 2026-06-30
tags: ["dictionary","jargon","glossary","terms","programming","vocabulary"]
---

इस wiki में उपयोग किए जाने वाले technical jargon, programming concepts, और command-line terms का एक central glossary (शब्दकोश)। नए शब्दों को सीखने और cross-reference करने के लिए इसका उपयोग करें।

<div class="dict-nav">

<a href="#letter-A" class="dict-letter-link">A</a> <a href="#letter-B" class="dict-letter-link">B</a> <a href="#letter-C" class="dict-letter-link">C</a> <a href="#letter-D" class="dict-letter-link">D</a> <a href="#letter-F" class="dict-letter-link">F</a> <a href="#letter-G" class="dict-letter-link">G</a> <a href="#letter-K" class="dict-letter-link">K</a> <a href="#letter-L" class="dict-letter-link">L</a> <a href="#letter-M" class="dict-letter-link">M</a> <a href="#letter-N" class="dict-letter-link">N</a> <a href="#letter-P" class="dict-letter-link">P</a> <a href="#letter-R" class="dict-letter-link">R</a> <a href="#letter-S" class="dict-letter-link">S</a> <a href="#letter-T" class="dict-letter-link">T</a> <a href="#letter-V" class="dict-letter-link">V</a>

</div>

## A {#letter-A}

### Acme.sh {#acme-sh}

Let's Encrypt या ZeroSSL के ज़रie SSL/TLS certificates automatically issue और renew करने के लिए एक zero-dependency Shell Script-based ACME client.


### Alacritty {#alacritty}

Rust language में लिखा गया एक ultra-fast, lightweight, और GPU-accelerated (hardware-accelerated) terminal emulator.


### API (Application Programming Interface) {#api}

अलग-अलग software applications/components के बीच data exchange और communication स्थापित करने का standard माध्यम।


### Assembly Language {#assembly-language}

एक low-level programming language जो computer architecture के machine instructions से closely map करती है, जिसे assembler के द्वारा easily assemble किया जा सकता है।


### Async (Asynchronous) {#async}

वह execution flow जिसमें task background में run होता है और main thread (UI loop) को block नहीं करता, जिससे user interface lag-free रहता है।


### AUR (Arch User Repository) {#aur}

Arch Linux users के लिए एक community-driven package repository, जहाँ user-built Custom packages की recipes (PKGBUILD) store होती हैं।




## B {#letter-B}

### Bootloader {#bootloader}

System hardware startup initialization के बाद operating system kernel (जैसे vmlinuz) को memory RAM में load करने वाला startup software tool (जैसे GRUB)।




## C {#letter-C}

### Callout {#callout}

Document layout में एक distinct styled container block, जो specific colors और dynamic icons के साथ important warnings, notes, or success logs को highlight करता है।


### CLI (Command Line Interface) {#cli}

Terminal console पर typing commands के द्वारा user interactions और computer file system operations को perform करने का interface.


### Commit {#commit}

Version control systems (जैसे Git) में codebase changes का snapshot local history में snapshot-id के साथ save करना।


### Consortium {#consortium}

विभिन्न organizations, companies या vendors का एक group जो common standards, goals या technology definitions (जैसे UNIX standards) को define करने के लिए collaborate करता है।


### Copyright {#copyright}

एक software codebase, design या literary creation पर owner का legal exclusive ownership right, जो unauthorized copy, reuse या modifications को restrict करता है।


### Cron {#cron}

Linux/Unix background service daemon (crond) जो user-defined scheduled intervals (crontab) पर script tasks या backup commands को automatically execute करता है।




## D {#letter-D}

### Daemon {#daemon}

Linux/Unix background service या process, जो user interaction के बिना automatically system tasks और events monitoring handle करती है।


### Dependency {#dependency}

किसी application program या tool execution के लिए mandatory components, software libraries, packages या scripts.


### Distribution (Distro) {#distribution-distro}

Linux kernel के साथ default command utilities, libraries, user environments, और package managers को wrap करके distribute किया जाने वाला complete operating system component (जैसे Debian, Slackware, Red Hat)।


### DOM (Document Object Model) {#dom}

HTML nodes का tree structure structure, जिसे JavaScript scripts runtime पर dynamically query, alter, style, या delete कर सकते हैं।




## F {#letter-F}

### FHS (Filesystem Hierarchy Standard) {#fhs}

Linux/Unix systems के root directory layout और folder structure roles को standardize करने वाला rules protocol guidelines.


### Fstab (/etc/fstab) {#fstab}

Linux system filesystems table configuration, जो system boot sequence के समय partitions (जैसे Root, Home, EFI) को specific directories/paths पर dynamically mount करती है।


### Fuzzy Search {#fuzzy-search}

Exact key matching के बजाय logical approximations, spelling mistakes tolerance, और prefix query inputs के आधार पर accurate matching results ढूँढने की method.




## G {#letter-G}

### Git {#git}

एक open-source, distributed version control application जो source repositories में file changes, revisions, और branches merging timeline को track करती है।


### GPL (General Public License) {#gpl}

GNU project द्वारा create किया गया एक standard copyleft open-source software license, जो users को software run, study, modify, और distribute करने की legal permission प्रदान करता है।




## K {#letter-K}

### Kernel {#kernel}

Operating system का core component जो computer hardware resources (CPU, RAM, devices) और processes runtime execution के बीच central mediator/management का काम करता है।




## L {#letter-L}

### Lawsuit {#lawsuit}

दो groups (जैसे USL vs. BSDI) के बीच copyright infringement, patents या trademark conflicts के solution के लिए court of law में दायर की जाने वाली legal case proceeding।


### LocalStorage {#localstorage}

Web browsers का key-value pair database engine, जो dynamic site properties (जैसे UI theme state) को session/tabs resets के बाद भी permanent save रखता है।




## M {#letter-M}

### MiniSearch {#minisearch}

Client-side execution structures के लिए optimized, pure JavaScript full-text indexing engine library, जो tag query processing और fuzzy searches support करता है।


### Mount {#mount}

किसी storage device (जैसे USB, SSD) या partition को system operating system के directory structure branch के path से securely connect करने की procedure.




## N {#letter-n}

### NFS (Network File System) {#nfs}

एक distributed file system protocol जो users को network पर remote folders/files access और mount करने की capability प्रदान करता है।


### Nim {#nim}

एक statically typed programming language, जो directly zero-overhead compiled C/C++ native code output produce करती है और super-lightweight servers execute करती है।




## P {#letter-P}

### Pacman {#pacman}

Arch Linux OS का package management CLI tool, जो database dependencies sync करने और system applications upgrades compile करने में use होता है।


### Pipes {#pipes}

Unix/Linux Shell का command-line pipe mechanism, जो एक output process (stdout) stream को सीधे दूसरी process के input (stdin) channel में redirect करता है।


### Portability {#portability}

किसी computer software या application की capability जिससे उसे without major code modifications के, different hardware architectures और systems पर execute किया जा सके।


### Promise {#promise}

JavaScript engine का object जो network fetch operations या execution wait timers के completion value (success or failure state) को represent करता है।




## R {#letter-R}

### Rebase {#rebase}

Git repository workflow model, जहाँ commits history timeline clean alignment structure maintain करने के लिए feature commits base sequence reset की जाती है।




## S {#letter-S}

### Static Site {#static-site}

HTML, CSS, और JS components से बनी website, जो servers par bina databases runtime computation run किए pre-built state assets direct render करती है।


### Symlink (Symbolic Link) {#symlink}

File system level पर dynamic file path redirects और links create करने वाली system directory shortcut file references specification.


### Systemd {#systemd}

Linux distributions init processes system component daemon, जो boot sequence bootstrap initialization processes control करता है और applications daemons state start/stop करता है।




## T {#letter-T}

### Trademark {#trademark}

किसी product, term (जैसे UNIX) या logo पर specific company/organization का registered intellectual property right, जो competitors को same term use करने से legal barriers में रोकता है।




## V {#letter-V}

### Variables {#variables}

Software programs memory containers, जो variable names के pointers structure के dynamically changing values, flags, array records list preserve रखते हैं।





